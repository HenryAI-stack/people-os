...
export default function DirectReports() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((i) => [i.name, i.role, i.team, i.location].join(' ').toLowerCase().includes(q))
  }, [items, query])
  ...
  async function handleDelete(id) {
    ...
    setError('')
    try { await directReportsStore.remove(id); load() }
    catch (e) { setError('Delete failed: ' + e.message) }
  }

  return (
    <>
      <div className="page-header"><h1>Direct Reports</h1><p>The people on your team — roles, tenure, and quick context at a glance.</p></div>
      <div className="toolbar">
        <input className="search-input" placeholder="Search by name, role, or team…" value={query} onChange={(e) => setQuery(e.target.value)} />
      ...
      {!loading && filtered.length === 0 && !error && <div className="empty-state"><div className="icon">👥</div>No direct reports yet.</div>}
      ...
      {grouped.map(({ team, members }) => (
        <div key={team || '__none__'} style={{ marginBottom:28 }}>
          ...
          <div className="list">
            {members.map((p) => {
              const fc = getCountryCode(p.location)
              return (
                  ...
                  <div className="row-main">
                    ...
                    <div>
                      ...
                      <div className="row-sub">
                        {p.role}
                        {p.location && <> · {p.location}{fc && <img src={flagUrl(fc)} alt={fc} style={{ width:20, height:15, objectFit:'cover', borderRadius:2, verticalAlign:'middle', marginLeft:4 }} />}</>}
                      </div>
                    </div>
                  </div>
      ...
      {editing !== null && (
        <ReportForm key={editing.id||'new'} initial={editing} onCancel={() => setEditing(null)} onSave={handleSave} />
      )}
    </>
  )
}
...
export function Avatar({ photo, name, size = 30 }) {
  ...
  if (photo) return <img src={photo} alt={name} style={{ width:size, height:size, borderRadius:'50%', objectFit:'cover', flexShrink:0, border:'1px solid var(--border)' }} />
  return (
    <div style={{ width:size, height:size, borderRadius:'50%', background:'var(--accent-soft)', color:'var(--accent)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:size*0.36, fontWeight:700, flexShrink:0, border:'1px solid var(--border)', userSelect:'none' }}>
      {initials}
    </div>
  )
}
...
function PhotoPicker({ value, onChange }) {
  ...
  async function handleFile(e) {
    ...
    setProcessing(true)
    try { onChange(await resizeImage(file, 200, 0.82)) }
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:16, marginBottom:18 }}>
      <div onClick={() => inputRef.current?.click()} style={{ width:72, height:72, borderRadius:'50%', overflow:'hidden', border:'2px dashed var(--border)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg)', flexShrink:0 }}>
        {value ? <img src={value} alt="preview" style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <span style={{ fontSize:26, opacity:0.4 }}>📷</span>}
      </div>
      <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
        <button type="button" className="btn" style={{ fontSize:12.5, padding:'6px 12px' }} onClick={() => inputRef.current?.click()} disabled={processing}>
          {processing ? 'Processing…' : value ? 'Change photo' : 'Upload photo'}
        </button>
        {value && <button type="button" className="btn ghost danger" style={{ fontSize:12.5, padding:'6px 12px' }} onClick={() => onChange('')}>Remove</button>}
        <span style={{ fontSize:11.5, color:'var(--text-faint)' }}>JPG, PNG, WebP — auto-cropped square</span>
      </div>
...
export function ReportForm({ initial, onCancel, onSave }) {
  ...
  return (
    <DraggableModal title={isNew ? 'Add direct report' : 'Edit direct report'} onClose={onCancel}>
      <form onSubmit={submit}>
        ...
        <div className="field"><label>Role / title</label><input value={form.role} onChange={(e) => set('role', e.target.value)} /></div>
        <div className="field"><label>Team</label><input value={form.team} onChange={(e) => set('team', e.target.value)} /></div>
        ...
        <div className="field"><label>Notes</label><textarea value={form.notes} onChange={(e) => set('notes', e.target.value)} placeholder="Strengths, growth areas, context…" /></div>
